...
% 3 Presenters: Shamsa / Saeeda / Amna
% Target: ~24 min (8 min each) + ~5 min Q&A
...
% ---------------- Presenter tag command ----------------
% Shows a small coloured "Presenter:" tag at top-right of every frame
\newcommand{\presenter}[2]{%
  \begin{tikzpicture}[remember picture,overlay]
    \node[anchor=north east, xshift=-0.3cm, yshift=-0.9cm,
          fill=#2!15, draw=#2, thick, rounded corners=2pt,
          text=#2, font=\scriptsize\bfseries, inner sep=3pt]
      at (current page.north east) {#1};
  \end{tikzpicture}%
}
\newcommand{\shamsa}{\presenter{Shamsa}{accentblue}}
\newcommand{\saeeda}{\presenter{Saeeda}{accentgreen}}
\newcommand{\amna}{\presenter{Amna}{accentorange}}

% ---------------- Title information ----------------
\title[Honest Bitcoin Forecasting]{Predicting Bitcoin Honestly}
\subtitle{Log-Return Targets, Walk-Forward Validation, \\ and Why Sub-1\% MAPE Isn't What It Seems}
\author[Alsuwaidi, Amer, Aljaroodi]{%
  \textbf{Shamsa Alsuwaidi} \and
  \textbf{Saeeda Amer} \and
  \textbf{Amna Aljaroodi}}
...
% =========== PRESENTERS SLIDE ===============================
\begin{frame}{Team \& Presentation Structure}
\begin{columns}[T,onlytextwidth]
\column{0.33\textwidth}
\begin{block}{\color{white}Part 1 --- Shamsa}
\textbf{Setup \& Motivation}
\begin{itemize}\small
  \item Problem statement
  \item Literature review
  \item Dataset overview
  \item The pipeline bug others miss
\end{itemize}
\end{block}

\column{0.33\textwidth}
\setbeamercolor{block title}{fg=white,bg=accentgreen}
\begin{block}{\color{white}Part 2 --- Saeeda}
\textbf{Methodology \& Models}
\begin{itemize}\small
  \item Stationary features
  \item Leak-free scaling
  \item Four architectures
...
\begin{block}{\color{white}Part 3 --- Amna}
\textbf{Evaluation \& Results}
\begin{itemize}\small
  \item Metric suite + baseline
  \item Static vs walk-forward
...
\vspace{0.6cm}
\centering
\footnotesize Each presenter's slides marked with a coloured name tag in the top-right corner.
\end{frame}
...
\section{Setup \& Motivation (Shamsa)}
...
\begin{itemize}
  ...
  \item Accessible via spot ETFs (SEC-approved January 2024)
...
\begin{frame}{The two papers we built on}
\shamsa
\begin{columns}[T,onlytextwidth]
\column{0.5\textwidth}
\setbeamercolor{block title}{fg=white,bg=accentblue}
...
\begin{itemize}\small
  ...
  \begin{itemize}\scriptsize
    ...
    \item News sentiment: FinBERT on GDELT headlines
  \end{itemize}
\end{itemize}
...
\vspace{0.6cm}
\small \textbf{Chronological split} --- no shuffling across the temporal boundary. Val and test are genuinely unseen future data.
\end{columns}
\end{frame}

\begin{frame}{Setup, summary}
\shamsa
\begin{itemize}
  \item Bitcoin's volatility, non-stationarity, and regime shifts make forecasting genuinely hard.
  \item Published MAPE figures span two orders of magnitude --- something is off with evaluation protocols.
  \item We identified \textbf{two silent bugs} common in the literature: scaler leakage and non-stationary targets.
  \item The fix is simple: predict \textbf{log returns}, fit the scaler on \textbf{training rows only}.
  \item Next: how we built the pipeline and the four architectures that compete inside it.
\end{itemize}

\vspace{0.6cm}
\centering
\Large \color{accentgreen}\textbf{Handing off to Saeeda} $\to$
\end{frame}
...
\section{Methodology \& Models (Saeeda)}
...
\begin{frame}{Model 1: DNN with feature gate}
\saeeda
\begin{columns}[T,onlytextwidth]
\column{0.5\textwidth}
...
\\[0.15cm] Re-weight input: $\mathbf{x}'_t = \mathbf{g} \odot \mathbf{x}_t$

\vspace{0.4cm}
\textbf{Architecture}
\\[0.1cm] Input (1020) $\to$ FC(512) + BN + ReLU + Dropout(0.3)
\\ $\to$ FC(256) + BN + ReLU + Dropout(0.3)
\\ $\to$ FC(128) + BN + ReLU + Dropout(0.2)
\\ $\to$ FC(64) + ReLU $\to$ FC(3)

\column{0.5\textwidth}
\textbf{Model 2: LSTM + temporal attention}
\vspace{0.1cm}
\begin{equation*}
\alpha_t = \frac{\exp(v^\top \tanh(W_a h_t))}{\sum_{s=1}^{60}\exp(v^\top \tanh(W_a h_s))}
\end{equation*}
\begin{equation*}
...
\begin{itemize}\small
  ...
  \item Learns which time steps matter
  \item FC head: 128 $\to$ 64 $\to$ 3
\end{itemize}
\end{columns}
\end{frame}

\begin{frame}{Model 3: CNN-Transformer}
\saeeda
\begin{columns}[T,onlytextwidth]
\column{0.55\textwidth}
\textbf{Stage 1: CNN local feature extraction}
\begin{itemize}\small
  \item Conv1D $17 \to 64$, kernel 3, MaxPool(2)
  \item Conv1D $64 \to 128$, kernel 3
  \item BatchNorm + ReLU + Dropout
  \item Compresses $60 \to 30$ time steps
\end{itemize}

\vspace{0.3cm}
\textbf{Stage 2: Transformer encoder}
\begin{itemize}\small
  \item Learned positional encoding (30 $\times$ 128)
...
\node[draw, fill=accentblue!20, minimum width=3.8cm, minimum height=0.55cm] (inp) {Input (60, 17)};
\node[draw, fill=accentgreen!20, minimum width=3.8cm, minimum height=0.55cm, below=of inp] (c1) {Conv1D $\to$ 64 + Pool};
\node[draw, fill=accentgreen!20, minimum width=3.8cm, minimum height=0.55cm, below=of c1] (c2) {Conv1D $\to$ 128};
\node[draw, fill=accentorange!20, minimum width=3.8cm, minimum height=0.55cm, below=of c2] (pe) {+ Positional Encoding};
\node[draw, fill=accentorange!20, minimum width=3.8cm, minimum height=0.55cm, below=of pe] (tf) {Transformer $\times$ 2};
\node[draw, fill=accentred!20, minimum width=3.8cm, minimum height=0.55cm, below=of tf] (fc) {Mean pool $\to$ FC $\to$ 3};
\draw[->] (inp) -- (c1);
\draw[->] (c1) -- (c2);
\draw[->] (c2) -- (pe);
\draw[->] (pe) -- (tf);
...
R = 1 + (k-1)(2^L - 1)
\end{equation*}
With $k=3$, $L=4$: $R = 31$ time steps.

\vspace{0.3cm}
\textbf{Residual block}
\begin{equation*}
\mathrm{out} = \mathrm{ReLU}(x + F(x))
\end{equation*}

\begin{itemize}\small
  \item Fully parallelizable across time
  \item No vanishing gradients
  \item 4 TemporalBlocks, 64 channels each
\end{itemize}
...
\foreach \y in {0,1,2,3} {
  \foreach \x in {0,...,7} {
    \fill[accentblue!\the\numexpr20+\y*15] (\x*0.5, \y*0.7) rectangle (\x*0.5+0.4, \y*0.7+0.3);
  }
}
\foreach \y [count=\i from 0] in {0,1,2,3} {
  \node[right] at (4.2, \y*0.7+0.15) {\tiny $d=2^{\i}$};
}
\node[below] at (2,-0.2) {\scriptsize Dilation doubles per layer};
\node[above] at (2, 3.2) {\scriptsize Output (last time step)};
\end{tikzpicture}

\vspace{0.3cm}
\small Pattern inspired by Bai, Kolter \& Koltun (2018) and Jin et al.\ (2025).
\end{columns}
\end{frame}

\begin{frame}{Training protocol and ensembling}
\saeeda
\begin{columns}[T,onlytextwidth]
\column{0.55\textwidth}
...
\begin{itemize}\small
  \item AdamW optimizer, lr $10^{-3}$, wd $10^{-4}$
...
\column{0.45\textwidth}
\textbf{Two ensembles, no oracle}
\vspace{0.1cm}
\\ \textbf{Simple Average:} mean of 4 model outputs
\vspace{0.15cm}
...
\begin{block}{Key distinction from CFA}
Weights come from \textbf{validation} only, never test. No per-day selection across combinations.
\end{block}
\end{columns}
\end{frame}
...
\saeeda
\begin{itemize}
  \item \textbf{Stationary features}: log returns, price/MA ratios, normalized MACD, sine-cosine halving.
  \item \textbf{Leak-free scaling}: StandardScaler fit on training rows only.
  \item \textbf{Multi-step target}: three simultaneous horizons (Day+1, +2, +3).
  \item \textbf{Four architectures}: DNN + gate, LSTM + attention, CNN-Transformer, TCN.
  \item \textbf{Unified training}: AdamW + Huber + cosine + early stopping for fair comparison.
  \item \textbf{Ensembles}: validation-weighted, no test-set information.
\end{itemize}

\vspace{0.7cm}
\centering
\Large \color{accentorange}\textbf{Handing off to Amna} $\to$
\end{frame}
...
\section{Evaluation \& Results (Amna)}
...
\column{0.55\textwidth}
\centering
\includegraphics[width=\linewidth]{fig06_walkforward_tcn.png}

\column{0.45\textwidth}
\textbf{Per-fold MAPE ranges from 0.99\% to 4.64\%}
\begin{itemize}\small
...
Volatile regimes (bull peaks, sharp drawdowns) dominate the error budget --- future work: regime-conditional models.
\end{block}
\end{columns}
\end{frame}
...
\vspace{1.5cm}
\normalsize
\textbf{Shamsa Alsuwaidi} \quad $\cdot$ \quad
\textbf{Saeeda Amer} \quad $\cdot$ \quad
\textbf{Amna Aljaroodi}
